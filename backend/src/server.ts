...
const focusPlanAnalyzeSchema = z.object({
...
const selectPetSkinSchema = z.object({
...
function focusCategoryForLabel(label: string) {
  if (containsAny(label, ['english', 'tiếng anh', 'tieng anh', 'vocabulary', 'ielts', 'toeic', 'listening', 'speaking'])) {
    return 'English';
  }
  if (containsAny(label, ['toán', 'toan', 'math', 'đạo hàm', 'dao ham', 'hình học', 'dai so', 'đại số'])) {
    return 'Math';
  }
  if (containsAny(label, ['ngữ văn', 'ngu van', 'văn', 'literature', 'essay', 'đọc hiểu', 'doc hieu'])) {
    return 'Literature';
  }
  if (containsAny(label, ['code', 'coding', 'flutter', 'programming', 'debug', 'api'])) {
    return 'Code';
  }
  if (containsAny(label, ['write', 'viết', 'viet', 'report', 'essay', 'draft'])) {
    return 'Write';
  }
  if (containsAny(label, ['ôn', 'học', 'hoc', 'study', 'review', 'prm', 'exam', 'quiz', 'luyện đề', 'luyen de'])) {
    return 'Review';
  }
  return 'Focus';
}
...
function detectSubjects(goal: string) {
  ...
  const entries = [
    { name: 'Math', keywords: ['toán', 'toan', 'math', 'đạo hàm', 'dao ham', 'hình học', 'hinh hoc', 'đại số', 'dai so'] },
    { name: 'English', keywords: ['english', 'tiếng anh', 'tieng anh', 'ielts', 'toeic', 'vocabulary', 'speaking', 'listening'] },
    { name: 'Literature', keywords: ['ngữ văn', 'ngu van', 'văn', 'literature', 'essay', 'đọc hiểu', 'doc hieu'] },
...
function detectAction(goal: string) {
  if (containsAny(goal, ['làm', 'lam', 'bài', 'bai', 'practice', 'luyện', 'luyen'])) return 'practice';
  if (containsAny(goal, ['ôn', 'review', 'kiểm tra', 'kiem tra'])) return 'review';
  if (containsAny(goal, ['thi', 'exam', 'quiz', 'luyện đề', 'luyen de'])) return 'exam';
  if (containsAny(goal, ['viết', 'viet', 'write', 'essay', 'draft'])) return 'write';
  if (containsAny(goal, ['đọc', 'doc', 'read'])) return 'read';
  return 'study';
}
...
app.post('/auth/register', async (req, res, next) => {
  try {
    ...
    res.status(201).json(await bootstrap(user.id));
  } catch (error) {
    next(error);
  }
});
