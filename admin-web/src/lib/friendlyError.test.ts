...
describe('friendlyError', () => {
  it('maps known HTTP statuses to Vietnamese messages', () => {
    // 401 with the synthetic 'Unauthorized' = expired/missing token → session-expired text.
    expect(friendlyError(new ApiError(401, 'Unauthorized'))).toBe('Phiên đăng nhập đã hết hạn. Vui lòng đăng nhập lại.');
  ...
  it('passes through a specific 401 message from the server (e.g. bad login)', () => {
    expect(friendlyError(new ApiError(401, 'Sai email hoặc mật khẩu quản trị'))).toBe('Sai email hoặc mật khẩu quản trị');
  });
